        ...
        return await self.try_call(
            "setMyCommands",
            {
                "commands": [
                    ...
                    {"command": "invite", "description": "Создать код доступа клиенту"},
                    {"command": "users", "description": "Список клиентов"},
                    {"command": "deluser", "description": "Удалить клиента"},
...
class HelperBot:
    ...
    def settings_keyboard(self, owner_user_id: int | None) -> dict[str, Any]:
        ...
        if self.is_superadmin(owner_user_id):
    ...
    async def handle_normal_message(self, message: dict[str, Any]) -> None:
        ...
        if text == "/id":
            ...
            if not self.is_superadmin(user_id):
                await self.api.send_message(chat_id, "Эта команда только для главного админа.")
                return
            await self.api.send_message(
                chat_id,
            ...
            if not self.is_admin(user_id):
                self.db.set_setting(f"register_state:{user_id}", "awaiting_code")
                await self.api.send_message(
                    chat_id,
                    "<b>Добро пожаловать в Chat Manager</b>\n\n"
                    "Это помощник для Telegram Business.\n\n"
                    "<b>Первый шаг:</b> отправьте одноразовый код приглашения. "
                    "Код должен выдать человек, который подключает вам бота.",
                    parse_mode="HTML",
                )
                return
            if text == "/start":
    ...
    async def handle_callback_query(self, query: dict[str, Any]) -> None:
        ...
        if data.startswith("admin:"):
            if not self.is_superadmin(from_user_id):
                if query_id:
